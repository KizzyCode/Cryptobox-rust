use crate::sodium::{
    crypto_secretbox_xchacha20poly1305_MACBYTES, crypto_secretbox_xchacha20poly1305_NONCEBYTES,
    crypto_secretbox_xchacha20poly1305_easy, crypto_secretbox_xchacha20poly1305_open_easy, randombytes_buf,
};
...
pub fn open(data: Vec<u8>, key: Vec<u8>) -> Vec<u8> {
    // Validate header
    if data.len() < HEADER.len() + NONCE_LEN {
        fail!(data_err => "Truncated header")
    }
    if !data.starts_with(HEADER) {
        fail!(data_err => "Unsupported header")
    }
    ...
    if data.len() < MAC_LEN {
        fail!(data_err => "Truncated stream")
    }
