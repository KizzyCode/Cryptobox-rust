...
macro_rules! fail {
    ...
    (api_err => $desc:expr) => (fail!(0x01, "user", $desc));
    (data_err => $desc:expr) => (fail!(0x02, "data", $desc));
    (internal_err => $desc:expr) => (fail!(0x10, "internal", $desc));
}
...
#[cfg(feature = "use-maproper")] #[global_allocator]
